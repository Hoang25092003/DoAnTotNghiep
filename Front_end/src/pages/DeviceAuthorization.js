...
function DevicesAuthorization() {
  ...
  const handleSave = async () => {
    try {
      ...
      console.log("Lỗi khi cập nhật phân quyền: ", err);
      toast.error("Cập nhật phân quyền thất bại! Vui lòng thử lại sau.");
    }
  };
  ...
  const handleDelete = async (row) => {
    try {
      ...
      toast.success("Đã xóa phân quyền thành công");
      fetchDevicesAuthorization();
    } catch (err) {
      toast.error("Xóa phân quyền thất bại! Vui lòng thử lại sau.");
    }
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (isDevice_AuthExist) {
        toast.warning("Thiết bị đang được sửa dụng vui lòng chọn thiết bị khác!");
        return;
      }
      await axios.post(`${process.env.REACT_APP_API_URL}/api/devicesAuth`, newRow, {
        withCredentials: true
      });
      toast.success("Đã thêm mới phân quyền thành công");
      setNewRow({
        device_id: "",
        user_id: ""
      });
      ...
      toast.error("Thêm mới phân quyền thất bại! Vui lòng thử lại sau.");
    }
  };
