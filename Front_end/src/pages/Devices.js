...
function Devices() {
  ...
  const handleDelete = async (device) => {
    ...
    if (window.confirm(`Bạn có chắc chắn muốn xóa thiết bị: ${device.device_id}?`)) {
      try {
        ...
        toast.error("Xóa thiết bị thất bại! Vui lòng thử lại sau.");
      }
    }
  };
  ...
  const handleAddDevice = async () => {
    try {
      ...
      if(isDeviceExist){
        toast.warning("Mã thiết bị đã tồn tại!");
        return;
      }
      ...
      toast.error("Thêm thiết bị thất bại! Vui lòng thử lại sau.");
      console.error(err);
    }
  };
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      toast.error("Cập nhật thiết bị thất bại! Vui lòng thử lại sau.");
    }
  };
