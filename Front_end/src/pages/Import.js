...
function Import() {
  ...
  useEffect(() => {
    ...
    const fetchDataSelectBox = async () => {
      try {
        ...
        console.error("Lỗi khi lấy dữ liệu select box:", error);
      } finally {
        setLoading(false);
      }
    };
    ...
    const startListening = () => {
      intervalId = setInterval(async () => {
        try {
          ...
          toast.error("Đã xảy ra lỗi khi quét mã vạch. Vui lòng thử lại.");
  ...
  const searchProducts = async (term) => {
    ...
    try {
      ...
      toast.error("Lôi tìm kiếm sản phẩm! Vui lòng thử lại.");
    }
  };
  ...
  const handleSelectProduct = (product) => {
    if (import_warehouses.length === 0) {
      toast.warning("Vui lòng chọn nhà kho trước khi thêm sản phẩm.");
      return;
    }
    setSelectedProducts((prevProducts) => {
  ...
  const handleConfirmImport = async () => {
    ...
    if (selectedProducts.length === 0) {
      toast.warning("Không có sản phẩm nào để nhập hàng.");
      return;
    }
